...
test "$(wasm-tools component wit fixtures/components/static.txt.wasm | grep -c 'import bloom:sign/signing@0.3.0' || true)" = 0
test "$(wasm-tools component wit fixtures/components/sign.wasm | grep -c 'import bloom:sign/signing@0.3.0' || true)" = 1
