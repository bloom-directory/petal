...
- A fixed set of choices is an enum, not strings compared with `==`. Use the
  typesystem. Where the SDK hands one back as a string —
  `OutboxInspection.state` — parse it into your enum at the boundary, not at
  every call site.
...
- **Compute on real Ethereum types** — alloy `Address`, `B256`, `U256`. Parse
  ...
  address bug; parsing is. Type the boundary end to end — a typed host trait,
  chain and status as enums — and render decimal strings only at the SDK call.
...
- A good test enforces new, non-trivial logic this code added, or a crucial
  ...
  keep. So does a lifecycle test that drives a route through a mock host.
...
- Map a host or HTTP error to `DispatchResponse` **once at the boundary** — one
  `SdkError` mapper, one HTTP-status mapper. Don't hand-translate the same
...
- Run the repository's own scripts, which wrap the vendored CLI at
  ...
  instead of `cargo install`). The binary is not on `$PATH`; without the scripts
  it is that vendored path, `build --root .` then `check --root .`, ending at
  the `bloom petals build` package gate when `bloom` is available.
...
- Adding a capability, host, path, or intent means editing `petal.toml`. **An
  ...
  failure from the stale-artifact `does not match its route source`; a clean
  rebuild will not clear it.
